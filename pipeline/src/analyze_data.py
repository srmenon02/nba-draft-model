...
from ingest.load_csv import load_base_data, split_train_test, get_data_summary
...
def analyze_by_draft_year(df: pd.DataFrame) -> pd.DataFrame:
    ...
    yearly.columns = ['players', 'nba_impact_mean', 'nba_impact_std', 'with_outcomes', 'international', 'avg_height', 'avg_age']
...
def analyze_correlations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    correlations = train_df[numeric_cols].corrwith(train_df['NBA_impact']).sort_values(ascending=False)
...
def generate_report(df: pd.DataFrame) -> str:
    ...
    report.append(stats[['mean', 'std', 'min', 'max', 'missing_pct']].head(10).to_string())
    ...
    if len(prospects_2026) > 0:
        ...
        report.append("-"*80)
        prospect_cols = ['name', 'pick', 'height', 'age', 'pts', 'ast', 'reb'] if 'reb' in prospects_2026.columns else ['name', 'pick', 'height', 'age']
...
def main():
    ...
    try:
        ...
        # Save report to file
        output_path = Path(__file__).parent.parent.parent / "data" / "processed" / "data_analysis_report.txt"
