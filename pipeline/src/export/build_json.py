...
def load_model_artifacts(models_dir: Path) -> Dict[str, Any]:
    ...
    if explanations_path.exists():
        ...
        print("✓ Loaded prospect explanations")
    ...
    if comparisons_path.exists():
        ...
        print("✓ Loaded player comparisons")
...
def build_prospects_json(
    df: pd.DataFrame, artifacts: Dict[str, Any], prospect_year: int = 2026
) -> List[Dict]:
    """
    ...
    for _, row in prospects_df.iterrows():
        ...
        prospect_dict = {
            ...
            "stats": {
                ...
                "rebounds": (
                    float(row["oreb"] + row["dreb"])
                    if pd.notna(row.get("oreb")) and pd.notna(row.get("dreb"))
                    else None
                ),
                ...
                "minutesPerGame": (
                    float(row["mpg"]) if pd.notna(row.get("mpg")) else None
                ),
            },
            # Efficiency metrics
            "metrics": {
                "trueShooting": (
                    float(row["ts_pct"]) if pd.notna(row.get("ts_pct")) else None
                ),
                "freeThrowPct": (
                    float(row["ft_pct"]) if pd.notna(row.get("ft_pct")) else None
                ),
                "threePointPct": (
                    float(row["three_pt_pct"])
                    if pd.notna(row.get("three_pt_pct"))
                    else None
                ),
                "rimPct": (
                    float(row["rim_pct"]) if pd.notna(row.get("rim_pct")) else None
                ),
                "assistToTurnover": (
                    float(row["ast_to_tov"])
                    if pd.notna(row.get("ast_to_tov"))
                    else None
                ),
            },
            # Predictions
...
def build_big_board_json(prospects_list: List[Dict]) -> List[Dict]:
    ...
    big_board = sorted(
        prospects_list, key=lambda x: x["prediction"]["nbaImpact"], reverse=True
    )
