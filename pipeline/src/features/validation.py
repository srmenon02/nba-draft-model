...
def check_missing_values(df: pd.DataFrame, critical_cols: Optional[List[str]] = None) -> Dict:
    """
...
def check_data_consistency(df: pd.DataFrame) -> Dict[str, List[str]]:
    ...
    for stat in counting_stats:
        if stat in df.columns and (df[stat] < 0).any():
            issues['negative_values'].append(f"{stat}: {(df[stat] < 0).sum()} negative values")
    ...
    if 'height' in df.columns:
        ...
        if unusual_heights.any():
            issues['height_issues'].append(f"Unusual heights: {unusual_heights.sum()} players")
...
def validate_training_data(
    ...
    if target_col in df.columns:
        ...
        if training_samples < 100:
            errors.append(f"Insufficient training samples: {training_samples} (need at least 100)")
...
def generate_validation_report(
    ...
    report_lines.append("-" * 80)
    numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    key_stats = [col for col in ['pts', 'ast', 'oreb', 'dreb', 'height', 'age', 'mpg'] 
...
if __name__ == "__main__":
    ...
    report_path = Path(__file__).parent.parent.parent.parent / "data/processed/validation_report.txt"
