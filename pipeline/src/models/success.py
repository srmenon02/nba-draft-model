...
def predict_prospects(
    model: Any, df: pd.DataFrame, feature_cols: List[str], prospect_year: int = 2026
) -> pd.DataFrame:
    """
    ...
    # Add prediction rank
    prospects["prediction_rank"] = prospects["predicted_NBA_impact"].rank(
        ascending=False
    )
    ...
    for i, row in top_10.iterrows():
        print(
            f"  {int(row['prediction_rank'])}. {row['name']} ({row['position']}): "
...
def save_model(model: Any, cv_results: Dict, feature_cols: List[str], output_dir: Path):
    """
