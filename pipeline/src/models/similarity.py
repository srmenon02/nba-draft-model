...
def prepare_similarity_features(
    ...
    print(f"✓ Features normalized by position")
...
def generate_all_prospect_comparisons(
    ...
    for array_idx, (df_idx, prospect) in enumerate(prospects.iterrows()):
        ...
        similar = find_similar_players(
            ...
            historical_features,
            historical.merge(
                historical_original[['name', 'season', 'NBA_impact', 'NBA_minutes', 'NBA_seasons']],
                on=['name', 'season'],
                how='left'
            ),
        ...
        for _, comp in similar.iterrows():
            comparisons.append({
                ...
                'nba_impact': float(comp['NBA_impact']) if pd.notna(comp['NBA_impact']) else None,
                'nba_seasons': int(comp['NBA_seasons']) if pd.notna(comp['NBA_seasons']) else None
            })
...
def print_prospect_comparisons(
    comparisons_dict: Dict[str, List[Dict]],
    top_prospects: int = 5
) -> str:
    """
    ...
    for prospect, comps in comparisons_dict.items():
        avg_impact = np.mean([c['nba_impact'] for c in comps if c['nba_impact'] is not None])
    ...
    for prospect, avg_impact in prospect_scores[:top_prospects]:
        ...
        for i, comp in enumerate(comps[:5], 1):
            impact_str = f"{comp['nba_impact']:.2f}" if comp['nba_impact'] is not None else "N/A"
            seasons_str = f"{comp['nba_seasons']} seasons" if comp['nba_seasons'] else "N/A"
            lines.append(
