...
def collect_combine_data(start_year: int = 2010, end_year: int = 2026) -> pd.DataFrame:
    ...
    if len(combine_df) > 0:
        ...
        print(f"\nSample data:")
        print(combine_df[['name', 'draft_year', 'position', 'wingspan', 'vertical_max', 'weight']].head())
...
def merge_supplemental_data(base_df: pd.DataFrame, combine_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    merged = base_df.merge(
        combine_df[['name_std', 'draft_year', 'position', 'height_wo_shoes', 'height_w_shoes', 
                    'weight', 'wingspan', 'standing_reach', 'vertical_max', 'lane_agility', 
    ...
    print(f"  New features added: position, wingspan, weight, vertical, etc.")
    
    # Show coverage by year
    coverage_by_year = merged.groupby('season').agg({
        'name': 'count',
        'wingspan': lambda x: x.notna().sum()
    })
    coverage_by_year['match_rate'] = (coverage_by_year['wingspan'] / coverage_by_year['name'] * 100).round(1)
...
def save_enhanced_dataset(df: pd.DataFrame, output_path: str = "data/processed/enhanced_draft_data.csv"):
    """
...
def generate_feature_summary(df: pd.DataFrame):
    ...
    features = {
        'Base Stats': ['ftm', 'fta', '3fgm', '3fga', 'rim_made', 'rim_miss', 'pts', 'ast', 'oreb', 'dreb', 'stl', 'blks'],
        'Context': ['height', 'age', 'gp', 'mpg', 'sos', 'team_strength', 'international'],
        'Combine Measurements': ['wingspan', 'weight', 'vertical_max', 'standing_reach', 'bench_press'],
        'Derived Metrics': ['ts_pct', 'ft_pct', 'three_pt_pct', 'rim_pct', 'ast_to_tov'],
        'Outcomes': ['NBA_impact', 'NBA_minutes', 'NBA_seasons']
    }
    
    for category, cols in features.items():
        available = [c for c in cols if c in df.columns]
        coverage = {col: f"{df[col].notna().sum()}/{len(df)} ({df[col].notna().sum()/len(df)*100:.1f}%)" 
