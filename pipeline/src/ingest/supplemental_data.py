...
class NBAApiCollector:
    ...
    def collect_combine_data_for_years(self, start_year: int, end_year: int) -> pd.DataFrame:
        """
...
def calculate_derived_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(col in df.columns for col in ['pts', '3fga', 'fta']):
        ...
        if 'rim_made' in df.columns and 'rim_miss' in df.columns:
            df['fga_est'] = df['3fga'] + (df['rim_made'] + df['rim_miss']) + (df.get('mid_made', 0) + df.get('mid_miss', 0))
    ...
    if 'dunks_made' in df.columns and 'rim_made' in df.columns:
        df['dunk_rate'] = df['dunks_made'] / (df['rim_made'] + df['rim_miss']).replace(0, 1)
...
if __name__ == "__main__":
    ...
    if collector.available:
        ...
        if len(combine_df) > 0:
            print(f"\nSample data:")
            print(combine_df[['PLAYER_NAME', 'POSITION', 'HEIGHT_WO_SHOES', 'WINGSPAN', 'VERTICAL_LEAP']].head())
    else:
